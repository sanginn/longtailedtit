...
/****************************************************************/
#ifndef FINITESTRAINCRITICALSTATEMOHRCOULOMB
#define FINITESTRAINCRITICALSTATEMOHRCOULOMB

#include "FiniteStrainPlasticBase2.h"

class FiniteStrainCriticalStateMohrCoulomb;
 ...
 */
class FiniteStrainCriticalStateMohrCoulomb : public FiniteStrainPlasticBase2
{
public:
  FiniteStrainCriticalStateMohrCoulomb(const InputParameters & parameters);
...
protected:
  ...
  Real _gamma;  // intercept of CSL in e-p' space
  Real _lambda; // slope of CSL in e-p' space
  Real _xi;     // exponential of CSL in e-p' space
  Real _p_ref;  // reference pressure (= 100kPa)
  Real _psi_0;  // constant to adjust dilatancy angle
  bool _fixed_elastic_moduli;
  Real _c_g;     // constant for the shear modulus
  Real _poisson_ratio;      // poisson's ratio
  ...
  /// Void ratio
  MaterialProperty<Real> & _void_ratio; // todo: later it should be a function
  ...
  virtual Real psi(const Real internal_param, const RankTwoTensor & stress);
